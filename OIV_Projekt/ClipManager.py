from tkinter import Tk, Label, RAISED, Frame, BOTH, Menu, SUNKEN, SOLID, messagebox
import pyperclip
...
class ClipboardManager(Frame):
    def __init__(self, parent=None):
        self.parent = parent
        Frame.__init__(self, parent, height=600, width=600)
        parent.title("Clipboard Manager Team13")
    ...
    def onClick(self, labelNum):
        ...
        self.after(ms=100, func=lambda label1=label: self.animateClick(label1))
    ...
    def clearAll(self):
        result = messagebox.askyesno("Clear All", "Are you sure you want to clear all clippings from the application?",
                                     icon='warning')
        if result:
...
if __name__ == '__main__':
    ...
    ClipboardManager.createLayout()
    ClipboardManager.updateClipboard()
    ClipboardManager.mainloop()
